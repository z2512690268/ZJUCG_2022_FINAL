...
index_t directedEdge2faceIndex(const directedEdge2indexMap_t& de2fi, index_t vi, index_t vj) {
    if (de2fi.empty()) {
        std::cout << "fail to build mesh: de2fi == empty()" << std::endl;
        return -1;
    }
    auto it = de2fi.find(std::make_pair(vi, vj));
    //没有对应面的he返回-1
    if(it == de2fi.end()) {
        if (de2fi.find(std::make_pair(vj, vi)) == de2fi.end()) {
            std::cout << "fail to build mesh: de2fi.find(std::make_pair(vj, vi)) == de2fi.end()" << std::endl;
            return -1;
        }
        return -1;
    }
...
int unorderedEdgesFromTriangles(const std::vector<triangle_t> &triangles, std::vector<edge_t> &edgesOut) {
    typedef std::set<std::pair<index_t , index_t > > edgeSet_t;       //边集，防止重复建边
    edgeSet_t edges;
    for (index_t t = 0; t < triangles.size(); ++t) {
        edges.insert(std::make_pair(std::min(triangles[t].i().index, triangles[t].j().index), std::max(triangles[t].i().index, triangles[t].j().index)));
        edges.insert(std::make_pair(std::min(triangles[t].j().index, triangles[t].k().index), std::max(triangles[t].j().index, triangles[t].k().index)));
        edges.insert(std::make_pair(std::min(triangles[t].k().index, triangles[t].i().index), std::max(triangles[t].k().index, triangles[t].i().index)));
    }
 ...
 * @return
 */
int trimesh_t::build(const std::vector<point_t >& points, const std::vector<edge_t>& edges, const std::vector<triangle_t>& triangles) {
    //有向边到面的映射
    directedEdge2indexMap_t de2fi;
    for (int fi = 0; fi < triangles.size(); ++fi) {
        const triangle_t& tri = triangles[fi];
        de2fi[std::make_pair(tri.v[0].index, tri.v[1].index)] = fi;
        de2fi[std::make_pair(tri.v[1].index, tri.v[2].index)] = fi;
        de2fi[std::make_pair(tri.v[2].index, tri.v[0].index)] = fi;
    }
    clear();
    ...
    for (int ei = 0; ei < edges.size(); ++ei) {
        ...
        if (mDirectedEdge2heIndex.find(std::make_pair(edge.v[0], edge.v[1])) != mDirectedEdge2heIndex.end()) {
            std::cout << "fail to build mesh: mDirectedEdge2heIndex.find(std::make_pair(edge.v[0], edge.v[1])) != mDirectedEdge2heIndex.end()" << std::endl;
            return -1;
        }
        if (mDirectedEdge2heIndex.find(std::make_pair(edge.v[1], edge.v[0])) != mDirectedEdge2heIndex.end()) {
            std::cout << "fail to build mesh: mDirectedEdge2heIndex.find(std::make_pair(edge.v[1], edge.v[0])) != mDirectedEdge2heIndex.end()" << std::endl;
            return -1;
        }
        //根据有向边可以寻找到对应的halfEdge
        mDirectedEdge2heIndex[std::make_pair(edge.v[0], edge.v[1])] = he0index;
        mDirectedEdge2heIndex[std::make_pair(edge.v[1], edge.v[0])] = he1index;
        ...
        if(mEdgeHalfEdges[ei] != -1) {
            std::cout << "fail to build mesh: mEdgeHalfEdges[ei] != -1" << std::endl;
            return -1;
        }
        mEdgeHalfEdges[ei] = he0index;
    }

    std::vector<index_t > boundaryHeis;
    for (index_t hei = 0; hei < mHalfEdges.size(); ++hei) {
        ...
        if(j == -1) {
            std::cout << "fail to build mesh: Caused by next halfEdge not found exception" << std::endl;
            return -1;
        }
        he.nextHe = mDirectedEdge2heIndex[std::make_pair(i, j)];
    }
    //对于边界的he，将整个边界路径找到
    std::map<index_t , std::set<index_t > > vertex2outgoingBoundaryHei;
    ...
    for (auto hei = boundaryHeis.begin(); hei != boundaryHeis.end(); hei++) {
        halfedge_t& he = mHalfEdges[*hei];
        std::set<index_t >& outgoing = vertex2outgoingBoundaryHei[he.toVertex];
        if (!outgoing.empty()) {
    ...
    for (auto it = vertex2outgoingBoundaryHei.begin(); it != vertex2outgoingBoundaryHei.end(); it++) {
        if(!it->second.empty()) {
            std::cout << "fail to build mesh: Caused by border redundant" << std::endl;
            return -1;
        }
    }
    std::cout << "successfully built the mesh" << std::endl;
    return 0;
}
/**
 * 找出边界上的所有顶点
 * @return 返回边界点集
 */
    int trimesh_t::boundaryVertices(std::vector<index_t>& v) const {
    ...
    v = std::vector<index_t >(result.begin(), result.end());
    return 0;
}
/**
 * 找出边界
 ...
 * @return
 */
int trimesh_t::boundaryEdges(std::vector<std::pair<index_t , index_t > >& result) {
    result.clear();
    index_t flag = 0, temp;
    ...
    if (flag >= mHalfEdges.size()) {
        std::cout << "no boundary have been found" << std::endl;
        return -1;
    }
...
const std::vector<point_t> &trimesh_t::getMPoints() const {
    return mPoints;
}

const std::vector<trimesh_t::halfedge_t> &trimesh_t::getMHalfEdges() const {
    return mHalfEdges;
}

const std::vector<index_t> &trimesh_t::getMVertexHalfEdges() const {
    return mVertexHalfEdges;
}

const std::vector<index_t> &trimesh_t::getMFaceHalfEdges() const {
    return mFaceHalfEdges;
}

const std::vector<index_t> &trimesh_t::getMEdgeHalfEdges() const {
    return mEdgeHalfEdges;
}

const std::vector<triangle_t> &trimesh_t::getMTriangles() const {
    return mTriangles;
}